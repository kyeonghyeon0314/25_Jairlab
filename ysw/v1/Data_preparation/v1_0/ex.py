#!/usr/bin/env python3
import os
import numpy as np
import argparse
from glob import glob
import pypcd

def quaternion_matrix(quat):
    """
    Convert a quaternion [qx, qy, qz, qw] to a 3x3 rotation matrix.
    """
    q = np.array(quat, dtype=np.float64)
    n = q[0]**2 + q[1]**2 + q[2]**2 + q[3]**2
    if n == 0:
        return np.identity(3)
    s = 2.0 / n
    return np.array([
        [1 - s * (q[1]**2 + q[2]**2),      s * (q[0]*q[1] - q[2]*q[3]), s * (q[0]*q[2] + q[1]*q[3])],
        [s * (q[0]*q[1] + q[2]*q[3]),      1 - s * (q[0]**2 + q[2]**2), s * (q[1]*q[2] - q[0]*q[3])],
        [s * (q[0]*q[2] - q[1]*q[3]),      s * (q[1]*q[2] + q[0]*q[3]), 1 - s * (q[0]**2 + q[1]**2)]
    ])

def process_trajectory(traj_file, output_pose_file):
    """
    Read the trajectory file (with lines like: 
       #timestamp x y z q_x q_y q_z q_w)
    and write a poses.txt file where each line is formatted as:
       R11 R12 R13 T1 R21 R22 R23 T2 R31 R32 R33 T3
    """
    with open(traj_file, 'r') as f_in, open(output_pose_file, 'w') as f_out:
        for line in f_in:
            if line.startswith('#'):
                continue
            parts = line.strip().split()
            if len(parts) != 8:
                continue
            # Extract translation (x, y, z) and quaternion components (qx, qy, qz, qw)
            x, y, z = map(float, parts[1:4])
            qx, qy, qz, qw = map(float, parts[4:8])
            R = quaternion_matrix([qx, qy, qz, qw])
            # Format as KITTI pose: 3x3 rotation matrix followed by translation vector.
            pose_line = f"{R[0,0]} {R[0,1]} {R[0,2]} {x} " \
                        f"{R[1,0]} {R[1,1]} {R[1,2]} {y} " \
                        f"{R[2,0]} {R[2,1]} {R[2,2]} {z}"
            f_out.write(pose_line + "\n")

def convert_pcd_to_bin(pcd_file, bin_file):
    """
    Load a .pcd file using pypcd and convert it to a .bin file in KITTI format.
    Each point is represented as 4 float32 values (x, y, z, intensity).
    If intensity is not available in the data, zeros will be used.
    """
    # pypcd may throw a TypeError regarding bytes vs. string,
    # so ensure you apply the patch described above.
    pc = pypcd.PointCloud.from_path(pcd_file)
    data = pc.pc_data
    # Extract x, y, z fields
    x = data['x'].astype(np.float32)
    y = data['y'].astype(np.float32)
    z = data['z'].astype(np.float32)
    if 'intensity' in data.dtype.names:
        intensity = data['intensity'].astype(np.float32)
    else:
        intensity = np.zeros_like(x)
    points = np.column_stack((x, y, z, intensity))
    points.tofile(bin_file)

def process_sequence(seq_id, input_dir, output_dir):
    """
    Process one sequence whose ID is given as a two-digit string.
    Expects:
      - A trajectory file: input_dir/traj_{seq_id}.txt
      - PCD files in folder: input_dir/{seq_id}/
    Generates:
      - poses.txt in output_dir/sequences/{seq_id}/
      - .bin files in output_dir/sequences/{seq_id}/ouster/
    """
    traj_file = os.path.join(input_dir, f"traj_{seq_id}.txt")
    pcd_folder = os.path.join(input_dir, seq_id)
    output_seq_dir = os.path.join(output_dir, "sequences", seq_id)
    ouster_dir = os.path.join(output_seq_dir, "ouster")
    os.makedirs(ouster_dir, exist_ok=True)
    
    # Process trajectory for poses.txt
    output_pose_file = os.path.join(output_seq_dir, "poses.txt")
    if os.path.exists(traj_file):
        process_trajectory(traj_file, output_pose_file)
        print(f"Processed trajectory for sequence {seq_id}")
    else:
        print(f"Warning: Missing trajectory file for sequence {seq_id}")
    
    # Find and convert all .pcd files
    pcd_files = sorted(glob(os.path.join(pcd_folder, "*.pcd")))
    for idx, pcd_file in enumerate(pcd_files):
        bin_filename = f"{idx:06d}.bin"
        bin_file_path = os.path.join(ouster_dir, bin_filename)
        try:
            convert_pcd_to_bin(pcd_file, bin_file_path)
        except Exception as e:
            print(f"Error converting file {pcd_file}: {e}")
    print(f"Converted {len(pcd_files)} PCD files to BIN for sequence {seq_id}")

def main():
    parser = argparse.ArgumentParser(
        description="Convert trajectory files to poses.txt and PCD files to KITTI .bin format."
    )
    parser.add_argument("input_dir", help="Input directory containing traj_XX.txt and sequence folders (e.g., '00', '01', …)")
    parser.add_argument("output_dir", help="Output directory where the sequences folder will be created")
    parser.add_argument("--num_seq", type=int, default=11, help="Number of sequences to process (default: 11)")
    args = parser.parse_args()
    
    for seq in range(args.num_seq):
        seq_str = f"{seq:02d}"
        process_sequence(seq_str, args.input_dir, args.output_dir)

if __name__ == "__main__":
    main()
